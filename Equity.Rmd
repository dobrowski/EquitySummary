...
ggplot( aes(x = reorder(definition, total ),
  ...
  geom_textbox(x = 3, y = 1500, fill = "lightgrey",
               label = "The majority of certified staff in Monterey County are White.  A quarter of certified staff are Latino and less than two percent are African American.")
...
coord_flip() + 
  ...
  geom_textbox(x = 5, y = 50000, fill = "lightgrey",
               label = "In contrast to the certified staff, the large majority of students are Hispanic or Latino. Latinxs represent four out of five students in public schools in Monterey County. Less than two percent of students are African-American.")
...
ap.county <- ap %>%
  ...
  mutate(perc.take = 100*numtsttakr/enroll1012,
         high.scorers = 100*(numscr3+numscr4+numscr5)/numtsttakr)
...
ap.county %>%
  filter(!is.na(high.scorers)) %>%
    ...
    labs(title = "AP Test Takers who Score 3, 4 or 5" ,
         caption = "Source: Postsecondary Preparation Data Files for 2019-20\nhttps://www.cde.ca.gov/ds/sp/ai/") +
...
ap.college.board %>%
  ...
  geom_textbox(x = 3, y = .35, fill = "lightgrey",
...
codebook.caaspp <- sbac.all %>%
    mutate(Subgroup_ID = as.character(Subgroup_ID),
           Students_Tested = as.numeric(Students_Tested)) %>%
...
caaspp_lollipop <- function(testy, district, students, df) {
   ...
   colorful <- case_when(district == FALSE ~ "orange",
    ...
    lollipop(y_var = Percentage_Standard_Met_and_Above,
             ...
             colorful) + 
  geom_richtext(size = 3,
              aes (label =  glue("*{comma( Percentage_Standard_Met_and_Above*Students_Tested/100, accuracy = 1)} of {comma(Students_Tested, accuracy = 1) }*" ) ),
              fill = NA, 
              label.color = NA,
              y = 0.04)  +
...
lollipop(
         ...
         colorme = "orange") +
                        geom_richtext(size = 3,
              aes (label = glue("*{comma(homeless.tot, accuracy = 1) } out of {comma(tot.tot, accuracy = 1) }*" ) ), 
              fill = NA, label.color = NA,
                   y = 0.32) +
  ...
  geom_textbox(x = 9, y = .20, fill = "lightgrey",
...
#  facet_grid(facets = vars(StudentGroupCategory), scales = "free" ) +
  ...
  mcoe_theme +
                        geom_richtext(size = 3,
              aes (label = glue("*{comma(foster.tot, accuracy = 1) } out of {comma(tot.tot, accuracy = 1) }*" ) ), 
              fill = NA, label.color = NA,
                   y = 8) +
  ...
  labs(title = ("Reported Foster Students per Thousand by District"),
       caption = "Source: CALPADS Unduplicated Pupil Count for 2020-21\n  https://www.cde.ca.gov/ds/ad/filescupc.asp") +
  geom_textbox(x = 9, y = 3, fill = "lightgrey",
...
lollipop(
         ...
         colorme = "orange") +
                        geom_richtext(size = 3,
              aes (label = glue("*{comma(migrant.tot, accuracy = 1) } out of {comma(tot.tot, accuracy = 1) }*" ) ), 
              fill = NA, label.color = NA,
                   y = 0.105) +

  labs(title = ("Percent of Migrant Students by District"),
       caption = "Source: CALPADS Unduplicated Pupil Count for 2020-21\n  https://www.cde.ca.gov/ds/ad/filescupc.asp") +
  geom_textbox(x = 10, y = .06, fill = "lightgrey",
